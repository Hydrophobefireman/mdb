...
function MetaData({ icon, text }) {
  return h(
    "div",
    { class: "movie-meta-item" },
    h("span", null, icon),
    h("span", { class: "meta-text" }, text)
  );
}
...
export function PosterBox({ data }) {
  ...
  const director = (data.credits || {}).director || "Unavailable";
  return h(
    "div",
    { class: "poster-box" },
    h(
    ...
    h(
      ...
      h(MetaData, {
        icon: genreIcon,
        text: genres.length ? genres.join(", ") : "N/A",
      }),
      h(MetaData, { icon: timeIcon, text: time }),
      h(MetaData, { icon: directorIcon, text: director })
    )
  );
}

const calenderIcon = h(
  "svg",
  {
    height: "24",
    width: "24",
